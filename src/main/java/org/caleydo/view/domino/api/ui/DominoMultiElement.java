 ...
 ******************************************************************************/
package org.caleydo.view.domino.api.ui;
...
import static org.caleydo.view.domino.api.ui.layout.EEdgeType.PARENT_CHILD;
import static org.caleydo.view.domino.api.ui.layout.EEdgeType.SHARED;
import static org.caleydo.view.domino.api.ui.layout.EEdgeType.SIBLING;
...
import org.caleydo.view.domino.spi.ui.layout.IGraphLayout;
import org.caleydo.view.domino.spi.ui.layout.IGraphLayout.GraphLayoutModel;
...
public class DominoMultiElement extends GLElement implements IHasMinSize, IGLElementParent,
 Iterable<DominoElement>,
	 ...
	 *
	 */
	public DominoMultiElement(MultiConfig config) {
	 ...
	 * @param layout
	 */
	public DominoMultiElement(IGraphLayout layout, MultiConfig config) {
		this.layout = layout;
		this.config = config;
	...
	public void toggleAlwaysShowHeader() {
		...
		for (DominoElement elem : Iterables.filter(this, DominoElement.class))
			elem.relayout();
	}
	...
	private void setup(DominoElement child) {
	 ...
	 */
	private void add(DominoElement child) {
		GraphVertex vertex = addImpl(child);
		createBands(vertex, ImmutableSet.of(child));
	}

	private GraphVertex addImpl(DominoElement child) {
		setup(child);
	...
	private void createBands(GraphVertex child, Set<DominoElement> ignores) {
	 ...
	 */
	private void split(DominoElement base, boolean inDim) {
		...
		List<DominoElement> children = toElements(base, inDim, baseVertex, table);

		// combine the elements that should be ignored
		final ImmutableSet<DominoElement> ignore = ImmutableSet.<DominoElement> builder().addAll(children)
				.add(base)
				.build();
		...
		for (int i = 0; i < children.size(); ++i) {
			DominoElement child = children.get(i);
	...
	private List<DominoElement> toElements(DominoElement base, boolean inDim, final GraphVertex baseVertex,
		...
		List<DominoElement> children = new ArrayList<>(datas.size());
		...
		for (TablePerspective t : datas) {
			final DominoElement new_ = new DominoElement(t, metaData, econfig, base);
			children.add(new_);
		}
		return children;
	}
	 ...
	 * @param crosswordElement
	 */
	public void splitDim(DominoElement child) {
		split(child, true);
	}

	public void splitRec(DominoElement child) {
		split(child, false);
	}

	@Override
	public boolean moved(GLElement child) {
		if (child instanceof DominoElement)
			removeVertex(child);
		relayout();
		return context != null;
	}

	private final Function<GraphVertex, DominoElement> toCrosswordElement = new Function<GraphVertex, DominoElement>() {
		@Override
		public DominoElement apply(GraphVertex input) {
			return input.asElement();
		}
	};

	@Override
	public Iterator<DominoElement> iterator() {
	...
	public void add(TablePerspective tablePerspective) {
		add(new DominoElement(tablePerspective, new TablePerspectiveMetaData(0, 0), config.getDefaultElementConfig()));
	}
	 ...
	 * @param crosswordElement
	 */
	public void onConnectionsChanged(DominoElement child) {
	...
	public void changePerspective(DominoElement child, boolean isDim, Perspective new_) {
		...
		if (context instanceof DominoView) {
			((DominoView) context).replaceTablePerspectiveInternally(old, newT);
		}
	}
	...
	private class GraphVertex implements IGraphVertex {
		...
		public GraphVertex(DominoElement element) {
			super();
			this.element = element;
		}

		public DominoElement asElement() {
			return element;
		}
		...
		private DominoLayoutInfo asLayoutInfo() {
			return element.getLayoutDataAs(DominoLayoutInfo.class, null);
		}
		...
		public Vec2f getSize() {
			DominoLayoutInfo info = asLayoutInfo();
			return info.getSize();
		}
